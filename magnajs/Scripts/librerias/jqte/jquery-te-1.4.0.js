...
(function($){		
	$.fn.jqte = function(options){
		...
		$.fn.jqteSetToInvalid = function (value, invalidClass) {
		    if (value) {
		        $(this).closest("." + vars.css).find("." + vars.css + "_editor").addClass(invalidClass);
		    }
		    else {
		        $(this).closest("." + vars.css).find("." + vars.css + "_editor").removeClass(invalidClass);
		    }
		}
		...
		return this.each(function(){
			...
			if (typeof (editor.context.isContentEditable) != 'undefined') {
			    isContentEditable = editor.context.isContentEditable;
			}


			// add feature editable to the text-field ve copy from the element's value to text-field
			editor.attr("contenteditable", isContentEditable).html(thisElementVal);
